...
![png](NEHA%20VERSION%20FULLY%20Khurrm%20bhaia_files/NEHA%20VERSION%20FULLY%20Khurrm%20bhaia_9_0.png)
...
def hitting_time_exact(P, target_state):
    ...
    # Get indices of all states except target
    idx = list(range(n))
    idx.remove(target_state)
    m = n - 1
    
    # Q = transitions between non-target states
    Q = P[np.ix_(idx, idx)]
    ...
    # Create full array with E[target] = 0
    full_E = np.zeros(n)
...
problem3_expected_steps_to_dispatch_from_dock = 4.85
```
