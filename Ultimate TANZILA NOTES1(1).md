...
```python
📋 SVD + Regression — SEPARATE TEMPLATES for Each Task Type
Here are separate templates for SVD and Regression, just like Markov Chains.

🎯 How to Know Which Template to Use
Problem	Exam Question Says...	Use Template
Problem 1	"Load data, do SVD, find explained variance, plot"	Template A
Problem 1	"Then do classification (nearest centroid)"	Template B
Problem 1	"Then do anomaly detection (reconstruction error)"	Template C
Problem 2	"Linear regression with metrics + Hoeffding"	Template D
Problem 2	"Cost thresholding + precision/recall + Hoeffding"	Template E
📋 PROBLEM 1: SVD — TEMPLATE A (ALWAYS DO THIS FIRST)
This is the BASE template. Use it for EVERY SVD problem.
...
# ==========================================
# STEP 2: SEPARATE X AND y (CHANGE COLUMN NAMES)
# ==========================================
# IF the file has a 'target' column:
problem1_X = df.drop(columns=['target']).to_numpy()  # <--- CHANGE 'target'
problem1_y = df['target'].to_numpy()  # <--- CHANGE 'target'

# IF the file has NO 'target' column (like SVD.csv in Jan exam):
# problem1_X = df.to_numpy()
# problem1_y = df['Label'].to_numpy()  # <--- CHANGE 'Label'
...
# STEP 6: FIND k (CHANGE ONLY THE THRESHOLD)
# ==========================================
THRESHOLD = 0.90  # <--- CHANGE THIS (0.90, 0.95, or 0.99)
problem1_num_components = np.argmax(problem1_explained_variance >= THRESHOLD) + 1
...
plt.grid(True)
plt.show()
📋 PROBLEM 1: SVD — TEMPLATE B (Classification)
Use this AFTER Template A when the exam asks for classification.

python
# ==========================================
# CONTINUE FROM TEMPLATE A
# ==========================================
# You already have: problem1_X, problem1_y, problem1_X_centered, 
# problem1_U, problem1_D, problem1_V, problem1_num_components

k = problem1_num_components

# ==========================================
# STEP 1: PROJECT DATA ONTO k COMPONENTS (NEVER CHANGE)
# ==========================================
problem1_scores_k = np.dot(problem1_X_centered, problem1_V[:, :k])

# ==========================================
# STEP 2: TRAIN/TEST SPLIT (80/20) (NEVER CHANGE)
# ==========================================
num_rows = problem1_scores_k.shape[0]
split_idx = int(num_rows * 0.8)
...
# ==========================================
# STEP 3: COMPUTE CENTROIDS (CHANGE ONLY THE NUMBER OF CLASSES)
# ==========================================
NUM_CLASSES = 10  # <--- CHANGE THIS (how many unique labels?)
...
# ==========================================
# STEP 4: PREDICT TEST SET (NEVER CHANGE)
# ==========================================
problem1_test_predictions = []
for test_row in X_test:
    distances = np.linalg.norm(problem1_centroids - test_row, axis=1)
...
# ==========================================
# STEP 5: CALCULATE ACCURACY (NEVER CHANGE)
# ==========================================
problem1_test_accuracy = np.mean(problem1_test_predictions == y_test)

print("Test Accuracy:", problem1_test_accuracy)
📋 PROBLEM 1: SVD — TEMPLATE C (Anomaly Detection)
Use this AFTER Template A when the exam asks for anomaly detection.
...
# ==========================================
# STEP 1: LOW-RANK APPROXIMATION (NEVER CHANGE)
# ==========================================
problem1_approximation = problem1_U[:, :k] @ problem1_D[:k, :k] @ problem1_V[:, :k].T

# ==========================================
# STEP 2: RECONSTRUCTION ERROR (NEVER CHANGE)
# ==========================================
problem1_reconstruction_error = np.linalg.norm(problem1_X_centered - problem1_approximation, axis=1)

# ==========================================
# STEP 3: PLOT EDF (NEVER CHANGE)
# ==========================================
sorted_errors = np.sort(problem1_reconstruction_error)
...
# ==========================================
# STEP 4: FIND THRESHOLD (CHANGE ONLY THE NUMBER OF OUTLIERS)
# ==========================================
NUM_OUTLIERS = 100  # <--- CHANGE THIS

sorted_errors = np.sort(problem1_reconstruction_error)
problem1_threshold = sorted_errors[-NUM_OUTLIERS]

# ==========================================
# STEP 5: IDENTIFY OUTLIERS (NEVER CHANGE)
# ==========================================
problem1_outliers = problem1_X[problem1_reconstruction_error >= problem1_threshold]
...
```python
📋 PROBLEM 2: Linear Regression — TEMPLATE D
Use this when the exam asks for Linear Regression + Metrics + Hoeffding.
...
# ==========================================
# STEP 2: CLEAN DATA (CHANGE THE COLUMN NAME)
# ==========================================
...
# ==========================================
# STEP 4: TRAIN/TEST SPLIT (80/20) (NEVER CHANGE)
# ==========================================
n = problem2_X.shape[0]
problem2_split_index = int(0.8 * n)
...
print("MSE:", problem2_mse_test)
print("MAE:", problem2_mae_test)
print("R²:", problem2_r2_test)
...
print("Baseline MSE:", problem2_baseline_mse_test)
print("Model beats baseline:", problem2_model_beats_baseline)

# ==========================================
# STEP 9: HOEFFDING (CHANGE ONLY THE BOUND)
# ==========================================
BOUND = 50  # <--- CHANGE THIS (what is the max possible value?)
...
print("Hoeffding 95% CI: [{:.4f}, {:.4f}]".format(problem2_lower_bound, problem2_upper_bound))
📋 PROBLEM 2: Cost Thresholding — TEMPLATE E
Use this when the exam asks for Cost Thresholding + Precision/Recall + Hoeffding (like Jan 2026).
...
# ==========================================
# STEP 1: DEFINE COSTS (CHANGE THESE FROM THE EXAM)
# ==========================================
COST_TP = 80    # <--- Cost of True Positive
COST_TN = 0     # <--- Cost of True Negative
COST_FP = 150   # <--- Cost of False Positive
COST_FN = 900   # <--- Cost of False Negative

# ==========================================
# STEP 2: COST FUNCTION (NEVER CHANGE)
# ==========================================
def problem2_avg_cost(y_true, y_predict_proba, threshold):
    y_true = np.asarray(y_true)
    y_pred = (np.asarray(y_predict_proba) >= threshold).astype(int)
    ...
    individual_costs = (COST_TP * tp.astype(float) + 
                        COST_TN * tn.astype(float) + 
                        COST_FP * fp.astype(float) + 
                        COST_FN * fn.astype(float))
    return float(np.mean(individual_costs))

# ==========================================
# STEP 3: FIND OPTIMAL THRESHOLD (NEVER CHANGE)
# ==========================================
# NOTE: You need y_true_val and y_pred_proba_val from the exam data
thresholds = np.arange(0, 1.01, 0.01)
costs = np.array([problem2_avg_cost(y_true_val, y_pred_proba_val, t) for t in thresholds])
...
print("Optimal threshold:", problem2_threshold)
print("Cost at optimal threshold:", problem2_cost_val)

# ==========================================
# STEP 4: PREDICT AT OPTIMAL THRESHOLD (NEVER CHANGE)
# ==========================================
problem2_y_pred_val = (y_pred_proba_val >= problem2_threshold).astype(int)

# ==========================================
# STEP 5: PRECISION AND RECALL (NEVER CHANGE)
# ==========================================
# For class 1 (positive)
problem2_precision_1 = precision_score(y_true_val, problem2_y_pred_val, pos_label=1, zero_division=0)
problem2_recall_1 = recall_score(y_true_val, problem2_y_pred_val, pos_label=1, zero_division=0)

# For class 0 (negative)
problem2_precision_0 = precision_score(y_true_val, problem2_y_pred_val, pos_label=0, zero_division=0)
problem2_recall_0 = recall_score(y_true_val, problem2_y_pred_val, pos_label=0, zero_division=0)
...
# ==========================================
# STEP 6: HOEFFDING FOR COST (CHANGE ONLY THE BOUND)
# ==========================================
BOUND = 900  # <--- CHANGE THIS (max possible cost - min possible cost)
...
individual_costs_test = (COST_TP * tp.astype(float) + 
                         COST_TN * tn.astype(float) + 
                         COST_FP * fp.astype(float) + 
                         COST_FN * fn.astype(float))

avg_test_cost = float(np.mean(individual_costs_test))
n_test = len(individual_costs_test)
alpha = 0.05
...
print("Test average cost:", avg_test_cost)
print("Hoeffding 95% CI: [{:.4f}, {:.4f}]".format(problem2_lower_bound, problem2_upper_bound))
📝 SUMMARY: What to Change in Each Template
Problem	Template	What to Change
1	A (Base SVD)	File name, column names, threshold
1	B (Classification)	Number of classes
1	C (Anomaly Detection)	Number of outliers
2	D (Regression)	File name, columns, target, bound
2	E (Cost Thresholding)	Costs, bound
⚠️ Quick Reference: Variable Names the Autograder Expects
Problem 1 Variables:
text
problem1_X
problem1_y
...
problem2_mae_test
problem2_r2_test
problem2_baseline_mse_test
problem2_model_beats_baseline
problem2_lower_bound
problem2_upper_bound
problem2_threshold
problem2_cost_val
problem2_precision_1
problem2_recall_1
problem2_precision_0
problem2_recall_0
```
